...
`;
    ...
    productosPagina.forEach(({ id, nombre, precio, imagen }) => {
        ...
        // 🔥 AUTO-FIX: Si la ruta es local, forzamos .webp para aprovechar la optimización
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            if (!rutaImagen.startsWith("http")) {
                ...
                // 🔥 AUTO-FIX: Forzamos .webp para el detalle
                rutaImagen = rutaImagen.replace(/\.(png|jpg|jpeg)$/i, ".webp");
            }
            const favsGuardados = obtenerFavoritos();
...
export function cargarProductosSimilares(productoActivo) {
    ...
    similares.forEach(({ id, nombre, precio, imagen }) => {
        ...
        // 🔥 AUTO-FIX: Forzamos .webp para similares
        if (!rutaImagen.startsWith("http")) {
            rutaImagen = rutaImagen.replace(/\.(png|jpg|jpeg)$/i, ".webp");
        }
    ...
    inputBusqueda.addEventListener("keyup", function (evento) {
        ...
        if (contenedorResultados) {
            ...
            primerosResultados.forEach(prod => {
                ...
                // 🔥 AUTO-FIX: Forzamos .webp en los resultados predictivos
                if (!rutaImg.startsWith("http")) {
                    rutaImg = rutaImg.replace(/\.(png|jpg|jpeg)$/i, ".webp");
                }
...
const imagenesHero = [
    "./img/banner-star-wars.webp",
    "./img/banner-naruto.webp",
    "./img/banner-marvel.webp"
];
let indiceActual = 0;
